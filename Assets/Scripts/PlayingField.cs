      ...
      // ondra
        ...
        if (IsGameOver())
        {
            if (ship.activeSkills[(int)ESkill.DEATH_SKIP]) {
                ...
                var text = !IsInPlayingField(shipX, shipY) ? "You strayed off course too much and your crew could not stand it anymore. It was time to change the captain." : "You ran out of food and the crew couldn't stomach it no more. They staged a full-on mutiny and tied you up to a  palm tree. Maybe next time!";
                textfield.text = text;
                Debug.Log(text);
